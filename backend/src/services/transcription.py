...
def _ambiguous_determiner_spans(text: str) -> set[tuple[int, int]]:
    """「あの/その」のうち、真の連体詞用法（直後に名詞等が続く＝フィラーでない）の
    char span を返す。

    unidic-lite の品詞タグ（連体詞 / 感動詞-フィラー）は「あの人」のような文脈でも揺れて
    信頼できないため、判定はタグに依らず「直後トークンが補助記号（読点等）か文末か」のみで行う。
...
@dataclass
class Word:
    """Whisper の word 粒度タイムスタンプ（話者帰属の分割点に使う中間表現）。

    full_text/fillers のオフセットは segment/full_text 基準のまま不変に保ち、
    words は **保存しない中間表現**として diarization の attribute_speakers にのみ渡す
    （docs/plans/005 §13.5/§13.16）。
    """
...
def _call_whisper(audio_path: str):
    ...
    with open(audio_path, "rb") as f:
        return client.audio.transcriptions.create(
            ...
            # word は話者境界での分割用、segment は従来の表示/フィラー基準用。
            timestamp_granularities=["word", "segment"],
        )


def _parse_whisper(resp) -> TranscriptionResult:
...
async def transcribe_verbose(audio_path: str) -> TranscriptionResult:
    """Transcript ＋ word タイムスタンプ（話者帰属用）を返す。"""
    import openai
    ...
    return _parse_whisper(resp)


async def transcribe(audio_path: str) -> Transcript:
    """後方互換の薄いラッパ（words が不要な呼び出し向け）。"""
    return (await transcribe_verbose(audio_path)).transcript
