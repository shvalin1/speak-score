...
Step2 の音声→文字起こし→評価パイプラインを、本番コード（`backend/src/services/`）に入れる前に
ローカルで試すための実験台。**本番ではない**（src への移植前の検証用）。

関連: [ADR 004](../../docs/adr/004_verbatim_transcription_and_disfluency.md) /
[実装計画 002](../../docs/plans/002_step2_real_pipeline.md) /
精度改善 issue: [#21](https://github.com/shvalin1/speak-score/issues/21)（担当 @hiroki428）
...
```sh
# 一気通貫
uv run --project backend python experiments/disfluency/run_chain.py --audio /path/to/speech.m4a

# 個別: フィラー保持の誘導あり/なし比較
uv run --project backend python experiments/disfluency/transcribe.py --audio speech.m4a
uv run --project backend python experiments/disfluency/transcribe.py --audio speech.m4a \
